...
func TestFilterCandidates(t *testing.T) {
	...
	testcases := []struct {
		...
		// Private IPs are filtered by default.
		{
...
func TestFilterSDPCandidates(t *testing.T) {
	...
	if filtered := FilterSDPCandidates(s, nil, nil); assert.True(filtered, "should have filtered") {
		for _, m := range s.MediaDescriptions {
			count := 0
			for _, a := range m.Attributes {
				if a.IsICECandidate() {
...
func TestNoFilterSDPCandidates(t *testing.T) {
	...
	if filtered := FilterSDPCandidates(s, nil, nil); assert.False(filtered, "should not have filtered") {
		for _, m := range s.MediaDescriptions {
			count := 0
			for _, a := range m.Attributes {
				if a.IsICECandidate() {
