...
func TestRoom_LowBandwidthNoPublisher(t *testing.T) {
	...
	pub.SetBandwidthInfo(&McuClientBandwidthInfo{
	...
	assert.EqualValues(0, pubs)
	assert.EqualValues(0, subs)
	assert.Equal(&McuClientBandwidthInfo{
		Sent:     api.BandwidthFromBits(0),
		Received: api.BandwidthFromBits(60000),
	}, bw)

	assert.EqualValues(0, pub.GetBandwidth())
	assert.EqualValues(1234567, room.GetNextPublisherBandwidth(StreamTypeScreen))
	assert.EqualValues(500000, room.GetNextPublisherBandwidth(StreamTypeVideo))
}
...
func TestRoom_Bandwidth(t *testing.T) {
	...
	room.Backend().bandwidthPerRoom.Store(0)
	room.Backend().minPublisherBandwidth.Store(0)
	room.Backend().maxPublisherBandwidth.Store(0)
	room.updateBandwidth().Wait()
	...
	assert.EqualValues(1234567, room.GetNextPublisherBandwidth(StreamTypeScreen))
	assert.EqualValues(700000, room.GetNextPublisherBandwidth(StreamTypeVideo))
}
...
func TestRoom_BandwidthClustered(t *testing.T) {
	...
	require.NotNil(pub1)
	pub1.SetBandwidthInfo(&McuClientBandwidthInfo{
		Sent:     api.BandwidthFromBits(0),
	...
	assert.EqualValues(600000, room1.GetNextPublisherBandwidth(StreamTypeScreen))
	assert.EqualValues(300000, room1.GetNextPublisherBandwidth(StreamTypeVideo))
	...
	require.True(client2.RunUntilAnswer(ctx, MockSdpAnswerAudioAndVideo))

	pub2 := mcu2.GetPublisher(hello2.Hello.SessionId)
	require.NotNil(pub2)
	pub2.SetBandwidthInfo(&McuClientBandwidthInfo{
		Sent:     api.BandwidthFromBits(0),
		Received: api.BandwidthFromBits(300000),
	})
	...
	assert.EqualValues(600000, room2.GetNextPublisherBandwidth(StreamTypeScreen))
	// The next publisher (the third one) will get limited bandwidths.
	assert.EqualValues(200000, room2.GetNextPublisherBandwidth(StreamTypeVideo))
	...
	assert.EqualValues(300000, pub1.GetBandwidth())
	assert.EqualValues(600000, room1.GetNextPublisherBandwidth(StreamTypeScreen))
	assert.EqualValues(200000, room1.GetNextPublisherBandwidth(StreamTypeVideo))
	...
	require.NoError(client3.SendMessage(MessageClientMessageRecipient{
		Type:      "session",
	...
	require.True(client3.RunUntilAnswer(ctx, MockSdpAnswerAudioAndVideo))

	pub3 := mcu1.GetPublisher(hello3.Hello.SessionId)
	require.NotNil(pub3)
	pub3.SetBandwidthInfo(&McuClientBandwidthInfo{
		Sent:     api.BandwidthFromBits(0),
	...
	assert.EqualValues(600000, room1.GetNextPublisherBandwidth(StreamTypeScreen))
	assert.EqualValues(150000, room1.GetNextPublisherBandwidth(StreamTypeVideo))
	assert.EqualValues(600000, room2.GetNextPublisherBandwidth(StreamTypeScreen))
	assert.EqualValues(150000, room2.GetNextPublisherBandwidth(StreamTypeVideo))
