...
const (
	...
	DefaultTargetType = TargetTypeStatic
...
func NewClient(logger log.Logger, target string, ip net.IP, opts ...grpc.DialOption) (*Client, error) {
...
func (c *Client) IsSelf() bool {
	return c.isSelf.Load()
}

func (c *Client) SetSelf(self bool) {
	c.isSelf.Store(self)
}

func (c *Client) GetServerId(ctx context.Context) (string, string, error) {
...
func (c *Client) LookupResumeId(ctx context.Context, resumeId api.PrivateSessionId) (*LookupResumeIdReply, error) {
...
func (c *Client) LookupSessionId(ctx context.Context, roomSessionId api.RoomSessionId, disconnectReason string) (api.PublicSessionId, error) {
...
func (c *Client) IsSessionInCall(ctx context.Context, sessionId api.PublicSessionId, roomId string, backendUrl string) (bool, error) {
	statsGrpcClientCalls.WithLabelValues("IsSessionInCall").Inc()
	// TODO: Remove debug logging
	c.logger.Printf("Check if session %s is in call %s on %s", sessionId, roomId, c.Target())
...
func (c *Client) GetInternalSessions(ctx context.Context, roomId string, backendUrls []string) (internal map[api.PublicSessionId]*InternalSessionData, virtual map[api.PublicSessionId]*VirtualSessionData, err error) {
...
func (c *Client) GetPublisherId(ctx context.Context, sessionId api.PublicSessionId, streamType sfu.StreamType) (api.PublicSessionId, string, net.IP, string, string, error) {
...
func (c *Client) GetSessionCount(ctx context.Context, url string) (uint32, error) {
...
func (c *Client) GetTransientData(ctx context.Context, roomId string, backend *talk.Backend) (api.TransientDataEntries, error) {
	...
	result := make(api.TransientDataEntries, len(entries))
	for k, v := range entries {
		...
		if v.Expires > 0 {
			result[k] = api.NewTransientDataEntryWithExpires(value, time.UnixMicro(v.Expires))
		} else {
			result[k] = api.NewTransientDataEntry(value, 0)
		}
	}
	return result, nil
}
...
func (c *Client) ProxySession(ctx context.Context, sessionId api.PublicSessionId, receiver ProxySessionReceiver) (*SessionProxy, error) {
...
type Clients struct {
	...
	// +checklocks:mu
	clientsMap map[string]*clientsList
	// +checklocks:mu
	clients []*Client
	...
	targetPrefix string
	// +checklocks:mu
	targetInformation map[string]*TargetInformationEtcd
...
func NewClients(ctx context.Context, config *goconf.ConfigFile, etcdClient etcd.Client, dnsMonitor *dns.Monitor, version string) (*Clients, error) {
...
func (c *Clients) GetServerInfoGrpc() (result []talk.BackendServerInfoGrpc) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *Clients) load(config *goconf.ConfigFile, fromReload bool) error {
...
func (c *Clients) closeClient(client *Client) {
	if client.IsSelf() {
		// Already closed.
		return
	}
...
func (c *Clients) isClientAvailable(target string, client *Client) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *Clients) getServerIdWithTimeout(ctx context.Context, client *Client) (string, string, error) {
...
func (c *Clients) WaitForSelfCheck() {
	c.selfCheckWaitGroup.Wait()
}

func (c *Clients) checkIsSelf(ctx context.Context, target string, client *Client) {
...
func (c *Clients) loadTargetsStatic(config *goconf.ConfigFile, fromReload bool, opts ...grpc.DialOption) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	clientsMap := make(map[string]*clientsList)
	var clients []*Client
	...
	for target := range internal.SplitEntries(targets, ",") {
		...
		if dnsDiscovery && net.ParseIP(host) == nil {
			...
			clientsMap[target] = &clientsList{
				entry: entry,
			}
			continue
		}

		client, err := NewClient(c.logger, target, nil, opts...)
		if err != nil {
...
func (c *Clients) onLookup(entry *dns.MonitorEntry, all []net.IP, added []net.IP, keep []net.IP, removed []net.IP) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	for _, ip := range added {
		client, err := NewClient(c.logger, target, ip, opts...)
		if err != nil {
	...
	if mapModified {
		...
		c.clients = make([]*Client, 0, len(c.clientsMap))
		for _, entry := range c.clientsMap {
...
func (c *Clients) loadTargetsEtcd(config *goconf.ConfigFile, fromReload bool, opts ...grpc.DialOption) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	if c.targetInformation == nil {
		c.targetInformation = make(map[string]*TargetInformationEtcd)
	}
...
func (c *Clients) EtcdClientCreated(client etcd.Client) {
	go func() {
...
func (c *Clients) EtcdWatchCreated(client etcd.Client, key string) {
}

func (c *Clients) getGrpcTargets(ctx context.Context, client etcd.Client, targetPrefix string) (*clientv3.GetResponse, error) {
...
func (c *Clients) EtcdKeyUpdated(client etcd.Client, key string, data []byte, prevValue []byte) {
	var info TargetInformationEtcd
	...
	opts := c.dialOptions.Load().([]grpc.DialOption)
	cl, err := NewClient(c.logger, info.Address, nil, opts...)
	if err != nil {
	...
	if c.clientsMap == nil {
		c.clientsMap = make(map[string]*clientsList)
	}
	c.clientsMap[info.Address] = &clientsList{
		clients: []*Client{cl},
	}
...
func (c *Clients) EtcdKeyDeleted(client etcd.Client, key string, prevValue []byte) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// +checklocks:c.mu
func (c *Clients) removeEtcdClientLocked(key string) {
...
func (c *Clients) WaitForInitialized(ctx context.Context) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
...
func (c *Clients) GetWakeupChannelForTesting() <-chan struct{} {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Clients) wakeupForTesting() {
	if c.wakeupChanForTesting == nil {
		return
	}
...
func (c *Clients) Reload(config *goconf.ConfigFile) {
...
func (c *Clients) GetClients() []*Client {
	...
	result := make([]*Client, 0, len(c.clients)-1)
	for _, client := range c.clients {
		if client.IsSelf() {
			continue
		}
