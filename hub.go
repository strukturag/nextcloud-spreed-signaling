...
func NewHub(config *goconf.ConfigFile, events AsyncEvents, rpcServer *GrpcServer, rpcClients *GrpcClients, etcdClient *EtcdClient, r *mux.Router, version string) (*Hub, error) {
	...
	if value, _ := config.GetString("mcu", "blockedcandidates"); value != "" {
		...
		log.Printf("Candidates blocklist: %s (and private / multicast addresses)", blocked)
		hub.blockedCandidates.Store(blocked)
	} else {
		log.Printf("No candidates blocklist, will only block private / multicast addresses")
	}
...
func (h *Hub) processMcuMessage(session *ClientSession, client_message *ClientMessage, message *MessageClientMessage, data *MessageClientMessageData) {
	...
	default:
		if data.Type == "candidate" && FilterCandidate(data.candidate, h.allowedCandidates.Load(), h.blockedCandidates.Load()) {
			log.Printf("Candidate %+v was blocked in session %s", data, session.PublicId())
			sendNotAllowed(session, client_message, "Candidate filtered.")
			return
		}
	...
	mc.SendMessage(session.Context(), message, data, func(err error, response map[string]interface{}) {
		if err != nil {
			...
			// We might receive the error message from the signaling proxy as string, so can't use "errors.Is".
			if err.Error() == ErrCandidateFiltered.Error() {
				sendNotAllowed(session, client_message, "Candidate filtered.")
			} else {
