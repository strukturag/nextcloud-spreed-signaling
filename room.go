...
func (r *Room) getRemoteBandwidth() (uint32, uint32, *McuClientBandwidthInfo) {
	if r.hub.rpcClients == nil {
		return 0, 0, nil
	}
	...
	for _, client := range r.hub.rpcClients.GetClients() {
		...
		go func(c *GrpcClient) {
			...
			pub, sub, bw, err := c.GetRoomBandwidth(ctx, r)
			if err != nil {
...
func (r *Room) GetNextPublisherBandwidth(streamType StreamType) api.Bandwidth {
	...
	bandwidthPerRoom := r.backend.bandwidthPerRoom.Load()
	// minPublisherBandwidth is the minimum bandwidth per publisher.
	minPublisherBandwidth := r.backend.minPublisherBandwidth.Load()
	// maxPublisherBandwidth is the maximum bandwidth per publisher.
	maxPublisherBandwidth := r.backend.maxPublisherBandwidth.Load()
	...
	if bandwidthPerRoom == 0 || minPublisherBandwidth == 0 || maxPublisherBandwidth == 0 {
		...
		for _, session := range publisherSessions {
			...
			go func() {
				...
				if err := session.UpdatePublisherBandwidth(ctx, StreamTypeVideo, bitrate); err != nil {
					r.logger.Printf("Could not update bandwidth of %s publisher in %s: %s", StreamTypeVideo, session.PublicId(), err)
				}
			}()
		}
		return &wg
		...
		if perPublisher != 0 {
			...
			for _, session := range publisherSessions {
				...
				go func() {
					...
					if err := session.UpdatePublisherBandwidth(ctx, StreamTypeVideo, perPublisher); err != nil {
						r.logger.Printf("Could not update bandwidth of %s publisher in %s: %s", StreamTypeVideo, session.PublicId(), err)
					}
				}()
			}
