	...
	for !in.IsDelim('}') {
		...
		case "entries":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v56 BackendPingEntry
					(v56).UnmarshalEasyJSON(in)
					out.Entries = append(out.Entries, v56)
					in.WantComma()
				}
				in.Delim(']')
			}
