...
func NewConfigStatic(logger log.Logger, config *goconf.ConfigFile, proxy McuProxy, dnsMonitor *dns.Monitor) (Config, error) {
	result := &configStatic{
		logger:         logger,
		proxy:          proxy,
...
func (p *configStatic) configure(cfg *goconf.ConfigFile, fromReload bool) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *configStatic) Start() error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *configStatic) Reload(config *goconf.ConfigFile) error {
	return p.configure(config, true)
}

func (p *configStatic) onLookup(entry *dns.MonitorEntry, all []net.IP, added []net.IP, keep []net.IP, removed []net.IP) {
	p.mu.Lock()
	defer p.mu.Unlock()
