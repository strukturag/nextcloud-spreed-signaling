...
func (c *proxyPubSubCommon) Sid() string {
	return c.sid
}

func (c *proxyPubSubCommon) StreamType() sfu.StreamType {
	return c.streamType
}
...
func (c *proxyPubSubCommon) doProcessPayload(client sfu.Client, msg *proxy.PayloadServerMessage) {
	switch msg.Type {
	case "offer":
...
func newProxyPublisher(logger log.Logger, id api.PublicSessionId, sid string, streamType sfu.StreamType, maxBitrate api.Bandwidth, settings sfu.NewPublisherSettings, proxyId string, conn *proxyConnection, listener sfu.Listener) *proxyPublisher {
	return &proxyPublisher{
		proxyPubSubCommon: proxyPubSubCommon{
			logger: logger,
...
func (p *proxyPublisher) GetConnectionURL() (string, net.IP) {
	return p.conn.rawUrl, p.conn.ip
}

func (p *proxyPublisher) PublisherId() api.PublicSessionId {
	return p.id
}

func (p *proxyPublisher) HasMedia(mt sfu.MediaType) bool {
	return (p.settings.MediaTypes & mt) == mt
}

func (p *proxyPublisher) SetMedia(mt sfu.MediaType) {
	// TODO: Also update mediaTypes on proxy.
	p.settings.MediaTypes = mt
}

func (p *proxyPublisher) NotifyClosed() {
	p.logger.Printf("Publisher %s at %s was closed", p.proxyId, p.conn)
	p.listener.PublisherClosed(p)
	p.conn.removePublisher(p)
}

func (p *proxyPublisher) Close(ctx context.Context) {
	p.NotifyClosed()
...
func (p *proxyPublisher) SendMessage(ctx context.Context, message *api.MessageClientMessage, data *api.MessageClientMessageData, callback func(error, api.StringMap)) {
	msg := &proxy.ClientMessage{
		Type: "payload",
...
func (p *proxyPublisher) ProcessPayload(msg *proxy.PayloadServerMessage) {
	p.doProcessPayload(p, msg)
}

func (p *proxyPublisher) ProcessEvent(msg *proxy.EventServerMessage) {
	switch msg.Type {
	case "ice-completed":
...
type proxySubscriber struct {
	...
	publisherConn *proxyConnection
}

func newProxySubscriber(logger log.Logger, publisherId api.PublicSessionId, sid string, streamType sfu.StreamType, maxBitrate api.Bandwidth, proxyId string, conn *proxyConnection, listener sfu.Listener, publisherConn *proxyConnection) *proxySubscriber {
...
func (s *proxySubscriber) GetConnectionURL() (string, net.IP) {
	return s.conn.rawUrl, s.conn.ip
}

func (s *proxySubscriber) Publisher() api.PublicSessionId {
	return s.publisherId
}

func (s *proxySubscriber) NotifyClosed() {
...
func (s *proxySubscriber) Close(ctx context.Context) {
	s.NotifyClosed()
...
func (s *proxySubscriber) SendMessage(ctx context.Context, message *api.MessageClientMessage, data *api.MessageClientMessageData, callback func(error, api.StringMap)) {
	msg := &proxy.ClientMessage{
		Type: "payload",
		Payload: &proxy.PayloadClientMessage{
			Type:     data.Type,
...
func (s *proxySubscriber) ProcessPayload(msg *proxy.PayloadServerMessage) {
	s.doProcessPayload(s, msg)
}

func (s *proxySubscriber) ProcessEvent(msg *proxy.EventServerMessage) {
	switch msg.Type {
	case "ice-completed":
...
type mcuProxyCallback func(response *proxy.ServerMessage)
...
type proxyConnection struct {
	...
	load       atomic.Uint64
	bandwidth  atomic.Pointer[proxy.EventServerBandwidth]
	mu         sync.Mutex
	...
	publishers map[string]*proxyPublisher
	// +checklocks:publishersLock
	publisherIds map[sfu.StreamId]api.PublicSessionId

	subscribersLock sync.RWMutex
	// +checklocks:subscribersLock
	subscribers map[string]*proxySubscriber
}

func newProxyConnection(proxy *proxySFU, baseUrl string, ip net.IP, token string) (*proxyConnection, error) {
	...
	conn := &proxyConnection{
		...
		publishers:   make(map[string]*proxyPublisher),
...
func (c *proxyConnection) IsSameCountry(initiator sfu.Initiator) bool {
	if initiator == nil {
		return true
	}
...
func (c *proxyConnection) IsSameContinent(initiator sfu.Initiator) bool {
	if initiator == nil {
		return true
	}
...
func (c *proxyConnection) GetStats() *proxyConnectionStats {
	result := &proxyConnectionStats{
		Url: c.url.String(),
		IP:  c.ip,
	}
	c.mu.Lock()
...
func (c *proxyConnection) Load() uint64 {
	return c.load.Load()
}

func (c *proxyConnection) Bandwidth() *proxy.EventServerBandwidth {
	return c.bandwidth.Load()
}

func (c *proxyConnection) Country() geoip.Country {
	return c.country.Load().(geoip.Country)
}

func (c *proxyConnection) Version() string {
...
func (c *proxyConnection) SessionId() api.PublicSessionId {
...
func (c *proxyConnection) setTemporary() {
	c.temporary.Store(true)
}

func (c *proxyConnection) clearTemporary() {
	c.temporary.Store(false)
}

func (c *proxyConnection) IsShutdownScheduled() bool {
	return c.shutdownScheduled.Load() || c.closeScheduled.Load()
}

func (c *proxyConnection) readPump() {
	defer func() {
		if !c.closed.Load() {
			c.scheduleReconnect()
		} else {
...
func (c *proxyConnection) sendPing() bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.conn == nil {
		return false
...
func (c *proxyConnection) writePump() {
...
func (c *proxyConnection) sendClose() error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *proxyConnection) stop(ctx context.Context) {
...
func (c *proxyConnection) stopCloseIfEmpty() {
	c.closeScheduled.Store(false)
}

func (c *proxyConnection) closeIfEmpty() bool {
...
func (c *proxyConnection) scheduleReconnect() {
	if err := c.sendClose(); err != nil && err != sfu.ErrNotConnected {
		c.logger.Printf("Could not send close message to %s: %s", c, err)
	}
	c.close()
...
func (c *proxyConnection) reconnect() {
...
func (c *proxyConnection) waitUntilConnected(ctx context.Context) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *proxyConnection) removePublisher(publisher *proxyPublisher) {
	...
	if _, found := c.publishers[publisher.proxyId]; found {
		...
		sfuinternal.StatsPublishersCurrent.WithLabelValues(string(publisher.StreamType())).Dec()
	}
	delete(c.publisherIds, sfu.GetStreamId(publisher.id, publisher.StreamType()))
...
func (c *proxyConnection) clearPublishers() {
	c.publishersLock.Lock()
	defer c.publishersLock.Unlock()

	go func(publishers map[string]*proxyPublisher) {
		for _, publisher := range publishers {
			publisher.NotifyClosed()
		}
	}(c.publishers)
	// Can't use clear(...) here as the map is processed by the goroutine above.
	c.publishers = make(map[string]*proxyPublisher)
...
func (c *proxyConnection) removeSubscriber(subscriber *proxySubscriber) {
	...
	if _, found := c.subscribers[subscriber.proxyId]; found {
		...
		sfuinternal.StatsSubscribersCurrent.WithLabelValues(string(subscriber.StreamType())).Dec()
	}
...
func (c *proxyConnection) clearSubscribers() {
	c.subscribersLock.Lock()
	defer c.subscribersLock.Unlock()

	go func(subscribers map[string]*proxySubscriber) {
		for _, subscriber := range subscribers {
			subscriber.NotifyClosed()
		}
	}(c.subscribers)
	// Can't use clear(...) here as the map is processed by the goroutine above.
	c.subscribers = make(map[string]*proxySubscriber)
...
func (c *proxyConnection) clearCallbacks() {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *proxyConnection) getCallback(id string) func(*proxy.ServerMessage) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *proxyConnection) registerDeferredCallback(msgId string, callback mcuProxyCallback) {
	if msgId == "" {
		return
	}
...
func (c *proxyConnection) getDeferredCallback(msgId string) mcuProxyCallback {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *proxyConnection) processMessage(msg *proxy.ServerMessage) {
...
func (c *proxyConnection) processPayload(msg *proxy.ServerMessage) {
	payload := msg.Payload
	c.publishersLock.RLock()
...
func (c *proxyConnection) processBye(msg *proxy.ServerMessage) {
	bye := msg.Bye
	switch bye.Reason {
...
func (c *proxyConnection) sendHello() error {
...
func (c *proxyConnection) sendMessage(msg *proxy.ClientMessage) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// +checklocks:c.mu
func (c *proxyConnection) sendMessageLocked(msg *proxy.ClientMessage) error {
	if proxyDebugMessages {
...
func (c *proxyConnection) performAsyncRequest(ctx context.Context, msg *proxy.ClientMessage, callback func(err error, response *proxy.ServerMessage)) string {
	msgId := strconv.FormatInt(c.msgId.Add(1), 10)
	msg.Id = msgId

	c.mu.Lock()
	defer c.mu.Unlock()
	c.callbacks[msgId] = func(msg *proxy.ServerMessage) {
		callback(nil, msg)
	}
...
func (c *proxyConnection) performSyncRequest(ctx context.Context, msg *proxy.ClientMessage) (*proxy.ServerMessage, string, error) {
	if err := ctx.Err(); err != nil {
		return nil, "", err
	}

	errChan := make(chan error, 1)
	responseChan := make(chan *proxy.ServerMessage, 1)
	msgId := c.performAsyncRequest(ctx, msg, func(err error, response *proxy.ServerMessage) {
		if err != nil {
			errChan <- err
		} else {
...
func (c *proxyConnection) deferredDeletePublisher(id api.PublicSessionId, streamType sfu.StreamType, response *proxy.ServerMessage) {
	if response.Type == "error" {
...
func (c *proxyConnection) newPublisher(ctx context.Context, listener sfu.Listener, id api.PublicSessionId, sid string, streamType sfu.StreamType, settings sfu.NewPublisherSettings) (sfu.Publisher, error) {
	msg := &proxy.ClientMessage{
		Type: "command",
		Command: &proxy.CommandClientMessage{
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			c.registerDeferredCallback(msgId, func(response *proxy.ServerMessage) {
	...
	publisher := newProxyPublisher(c.logger, id, sid, streamType, response.Command.Bitrate, settings, proxyId, c, listener)
	c.publishersLock.Lock()
	c.publishers[proxyId] = publisher
	c.publisherIds[sfu.GetStreamId(id, streamType)] = api.PublicSessionId(proxyId)
...
func (c *proxyConnection) deferredDeleteSubscriber(publisherSessionId api.PublicSessionId, streamType sfu.StreamType, publisherConn *proxyConnection, response *proxy.ServerMessage) {
	if response.Type == "error" {
...
func (c *proxyConnection) newSubscriber(ctx context.Context, listener sfu.Listener, publisherId api.PublicSessionId, publisherSessionId api.PublicSessionId, streamType sfu.StreamType) (sfu.Subscriber, error) {
	msg := &proxy.ClientMessage{
		Type: "command",
		Command: &proxy.CommandClientMessage{
	...
	subscriber := newProxySubscriber(c.logger, publisherSessionId, response.Command.Sid, streamType, response.Command.Bitrate, proxyId, c, listener, nil)
	c.subscribersLock.Lock()
...
func (c *proxyConnection) newRemoteSubscriber(ctx context.Context, listener sfu.Listener, publisherId api.PublicSessionId, publisherSessionId api.PublicSessionId, streamType sfu.StreamType, publisherConn *proxyConnection, remoteToken string) (sfu.Subscriber, error) {
	if c == publisherConn {
	...
	if remoteToken == "" {
		var err error
		if remoteToken, err = c.proxy.CreateToken(string(publisherId)); err != nil {
			return nil, err
		}
	}
	...
	subscriber := newProxySubscriber(c.logger, publisherSessionId, response.Command.Sid, streamType, response.Command.Bitrate, proxyId, c, listener, publisherConn)
	c.subscribersLock.Lock()
	c.subscribers[proxyId] = subscriber
	c.subscribersLock.Unlock()
	sfuinternal.StatsSubscribersCurrent.WithLabelValues(string(streamType)).Inc()
...
func newProxySettings(ctx context.Context, config *goconf.ConfigFile) (sfu.Settings, error) {
...
func (s *proxySettings) load(config *goconf.ConfigFile) error {
...
func (s *proxySettings) Reload(config *goconf.ConfigFile) {
	if err := s.load(config); err != nil {
		s.Logger.Printf("Error reloading proxy settings: %s", err)
	}
}

type proxySFU struct {
	logger   log.Logger
	urlType  string
	tokenId  string
	...
	// +checklocks:connectionsMu
	connectionsMap map[string][]*proxyConnection
	connRequests   atomic.Int64
	nextSort       atomic.Int64

	settings sfu.Settings
...
func NewProxySFU(ctx context.Context, config *goconf.ConfigFile, etcdClient etcd.Client, rpcClients *grpc.Clients, dnsMonitor *dns.Monitor) (sfu.SFU, error) {
	logger := log.LoggerFromContext(ctx)
	...
	settings, err := newProxySettings(ctx, config)
	if err != nil {
		return nil, err
	}
	...
	mcu := &proxySFU{
		...
		connectionsMap: make(map[string][]*proxyConnection),
		settings:       settings,

		publishers: make(map[sfu.StreamId]*proxyConnection),
	...
	case proxyUrlTypeStatic:
		mcu.config, err = NewConfigStatic(logger, config, mcu, dnsMonitor)
	case proxyUrlTypeEtcd:
		mcu.config, err = NewConfigEtcd(logger, config, etcdClient, mcu)
	default:
...
func (m *proxySFU) GetBandwidthLimits() (api.Bandwidth, api.Bandwidth) {
	return m.settings.MaxStreamBitrate(), m.settings.MaxScreenBitrate()
}

func (m *proxySFU) loadContinentsMap(cfg *goconf.ConfigFile) error {
...
func (m *proxySFU) Start(ctx context.Context) error {
	return m.config.Start()
}

func (m *proxySFU) Stop() {
...
func (m *proxySFU) CreateToken(subject string) (string, error) {
...
func (m *proxySFU) getConnections() []*proxyConnection {
	m.connectionsMu.RLock()
	defer m.connectionsMu.RUnlock()
...
func (m *proxySFU) ConnectionsCount() int {
	m.connectionsMu.RLock()
	defer m.connectionsMu.RUnlock()
...
func (m *proxySFU) hasConnections() bool {
	m.connectionsMu.RLock()
	defer m.connectionsMu.RUnlock()

	return slices.ContainsFunc(m.connections, func(conn *proxyConnection) bool {
		return conn.IsConnected()
	})
}

func (m *proxySFU) WaitForDisconnected(ctx context.Context) error {
...
func (m *proxySFU) WaitForConnections(ctx context.Context) error {
...
func (m *proxySFU) WaitForConnectionsEstablished(ctx context.Context, waitMap map[string]bool) error {
	m.connectionsMu.RLock()
	defer m.connectionsMu.RUnlock()
...
func (m *proxySFU) AddConnection(ignoreErrors bool, url string, ips ...net.IP) error {
	m.connectionsMu.Lock()
	defer m.connectionsMu.Unlock()

	var conns []*proxyConnection
	if len(ips) == 0 {
		conn, err := newProxyConnection(m, url, nil, "")
		if err != nil {
			if ignoreErrors {
		...
		for _, ip := range ips {
			conn, err := newProxyConnection(m, url, ip, "")
			if err != nil {
				if ignoreErrors {
	...
	for _, conn := range conns {
		...
		if existing, found := m.connectionsMap[url]; found {
			...
			m.connectionsMap[url] = []*proxyConnection{conn}
		}
	}
...
func (m *proxySFU) iterateConnections(url string, ips []net.IP) iter.Seq[*proxyConnection] {
	return func(yield func(*proxyConnection) bool) {
		m.connectionsMu.Lock()
		defer m.connectionsMu.Unlock()
...
func (m *proxySFU) RemoveConnection(url string, ips ...net.IP) {
...
func (m *proxySFU) KeepConnection(url string, ips ...net.IP) {
	for conn := range m.iterateConnections(url, ips) {
...
func (m *proxySFU) Reload(config *goconf.ConfigFile) {
...
func (m *proxySFU) removeConnection(c *proxyConnection) {
	m.connectionsMu.Lock()
	defer m.connectionsMu.Unlock()
...
func (m *proxySFU) SetOnConnected(f func()) {
	// Not supported.
}

func (m *proxySFU) SetOnDisconnected(f func()) {
	// Not supported.
}
...
type mcuProxyStats struct {
	Publishers int64                   `json:"publishers"`
	Clients    int64                   `json:"clients"`
	Details    []*proxyConnectionStats `json:"details"`
}

func (m *proxySFU) GetStats() any {
...
func (m *proxySFU) GetServerInfoSfu() *talk.BackendServerInfoSfu {
	m.connectionsMu.RLock()
	defer m.connectionsMu.RUnlock()
...
func (m *proxySFU) getContinentsMap() ContinentsMap {
...
func (m *proxySFU) setContinentsMap(continentsMap ContinentsMap) {
	if continentsMap == nil {
		continentsMap = make(ContinentsMap)
	}
...
type mcuProxyConnectionsList []*proxyConnection
...
func sortConnectionsForCountry(connections []*proxyConnection, country geoip.Country, continentMap ContinentsMap) []*proxyConnection {
...
func (m *proxySFU) getSortedConnections(initiator sfu.Initiator) []*proxyConnection {
	m.connectionsMu.RLock()
	...
	if m.connRequests.Add(1)%connectionSortRequests == 0 || m.nextSort.Load() <= now {
		...
		slices.SortFunc(sorted, func(a, b *proxyConnection) int {
...
func (m *proxySFU) removePublisher(publisher *proxyPublisher) {
	m.mu.Lock()
	defer m.mu.Unlock()

	delete(m.publishers, sfu.GetStreamId(publisher.id, publisher.StreamType()))
}

func (m *proxySFU) createPublisher(ctx context.Context, listener sfu.Listener, id api.PublicSessionId, sid string, streamType sfu.StreamType, settings sfu.NewPublisherSettings, initiator sfu.Initiator, connections []*proxyConnection, isAllowed func(c *proxyConnection) bool) sfu.Publisher {
...
func (m *proxySFU) NewPublisher(ctx context.Context, listener sfu.Listener, id api.PublicSessionId, sid string, streamType sfu.StreamType, settings sfu.NewPublisherSettings, initiator sfu.Initiator) (sfu.Publisher, error) {
	connections := m.getSortedConnections(initiator)
	publisher := m.createPublisher(ctx, listener, id, sid, streamType, settings, initiator, connections, func(c *proxyConnection) bool {
		bw := c.Bandwidth()
		return bw == nil || bw.AllowIncoming()
	})
	if publisher == nil {
		// No proxy has available bandwidth, select one with the lowest currently used bandwidth.
		connections2 := make([]*proxyConnection, 0, len(connections))
		for _, c := range connections {
			if c.Bandwidth() != nil {
				connections2 = append(connections2, c)
			}
		}
		slices.SortFunc(connections2, func(a *proxyConnection, b *proxyConnection) int {
		...
		publisher = m.createPublisher(ctx, listener, id, sid, streamType, settings, initiator, connections2, func(c *proxyConnection) bool {
			return true
		})
	}
...
func (m *proxySFU) getPublisherConnection(publisher api.PublicSessionId, streamType sfu.StreamType) *proxyConnection {
	m.mu.RLock()
	defer m.mu.RUnlock()

	return m.publishers[sfu.GetStreamId(publisher, streamType)]
}

func (m *proxySFU) waitForPublisherConnection(ctx context.Context, publisher api.PublicSessionId, streamType sfu.StreamType) *proxyConnection {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *proxySFU) createSubscriber(ctx context.Context, listener sfu.Listener, info *proxyPublisherInfo, publisher api.PublicSessionId, streamType sfu.StreamType, connections []*proxyConnection, isAllowed func(c *proxyConnection) bool) sfu.Subscriber {
	for _, conn := range connections {
		if !isAllowed(conn) || conn.IsShutdownScheduled() || conn.IsTemporary() {
			continue
		}
...
func (m *proxySFU) NewSubscriber(ctx context.Context, listener sfu.Listener, publisher api.PublicSessionId, streamType sfu.StreamType, initiator sfu.Initiator) (sfu.Subscriber, error) {
	...
	if conn := m.getPublisherConnection(publisher, streamType); conn != nil {
		...
		id, found := conn.publisherIds[sfu.GetStreamId(publisher, streamType)]
		...
		if clients := m.rpcClients.GetClients(); len(clients) > 0 {
			for _, client := range clients {
				...
				go func(client *grpc.Client) {
					...
					connections := m.connections
					m.connectionsMu.RUnlock()
					var publisherConn *proxyConnection
					for _, conn := range connections {
					...
					if publisherConn == nil {
						publisherConn, err = newProxyConnection(m, url, ip, connectToken)
						if err != nil {
						...
						if found {
							...
							conns = []*proxyConnection{publisherConn}
						}
	...
	if !allowOutgoing || !publisherInfo.conn.IsSameCountry(initiator) {
		...
		if !allowOutgoing || len(connections) > 0 && !connections[0].IsSameCountry(publisherInfo.conn) {
			...
			subscriber := m.createSubscriber(ctx, listener, publisherInfo, publisher, streamType, connections, func(c *proxyConnection) bool {
				bw := c.Bandwidth()
			...
			if subscriber == nil {
				...
				subscriber = m.createSubscriber(ctx, listener, publisherInfo, publisher, streamType, connections2, func(c *proxyConnection) bool {
					return true
				})
			}
			if subscriber != nil {
