...
func (p *janusPublisher) handleEvent(event *janus.EventMsg) {
...
func (p *janusPublisher) handleHangup(event *janus.HangupMsg) {
	p.logger.Printf("Publisher %d received hangup (%s), closing", p.handleId.Load(), event.Reason)
	go p.Close(context.Background())
}

func (p *janusPublisher) handleDetached(event *janus.DetachedMsg) {
...
func (p *janusPublisher) handleMedia(event *janus.MediaMsg) {
	mediaType := sfu.StreamType(event.Type)
	if mediaType == sfu.StreamTypeVideo && p.streamType == sfu.StreamTypeScreen {
...
func (p *janusPublisher) HasMedia(mt sfu.MediaType) bool {
	return (p.settings.MediaTypes & mt) == mt
}

func (p *janusPublisher) SetMedia(mt sfu.MediaType) {
	p.settings.MediaTypes = mt
}

func (p *janusPublisher) NotifyReconnected() {
	ctx := context.TODO()
...
func (p *janusPublisher) Close(ctx context.Context) {
	notify := false
	p.mu.Lock()
	if handle := p.handle.Load(); handle != nil && p.roomId != 0 {
		...
		p.mcu.mu.Lock()
		delete(p.mcu.publishers, sfu.GetStreamId(p.id, p.streamType))
		p.mcu.mu.Unlock()
		p.roomId = 0
		notify = true
	}
	...
	if notify {
		sfuinternal.StatsPublishersCurrent.WithLabelValues(string(p.streamType)).Dec()
...
func (p *janusPublisher) SendMessage(ctx context.Context, message *api.MessageClientMessage, data *api.MessageClientMessageData, callback func(error, api.StringMap)) {
...
func (p *janusPublisher) GetStreams(ctx context.Context) ([]sfu.PublisherStream, error) {
	...
	var streams []sfu.PublisherStream
...
func (p *janusPublisher) PublishRemote(ctx context.Context, remoteId api.PublicSessionId, hostname string, port int, rtcpPort int) error {
...
func (p *janusPublisher) UnpublishRemote(ctx context.Context, remoteId api.PublicSessionId, hostname string, port int, rtcpPort int) error {
	handle := p.handle.Load()
	if handle == nil {
		return sfu.ErrNotConnected
	}
