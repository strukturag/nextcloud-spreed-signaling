...
func Test_JanusPublisherSubscriberRace(t *testing.T) {
	t.Parallel()
	require := require.New(t)
	assert := assert.New(t)
	...
	waiter, release := mcu.newPublisherConnectedWaiter(pubId, sfu.StreamTypeVideo)
	...
	assert.NoError(waiter.Wait(ctx))
	release()
