...
func (h *TestJanusEventsServerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if conn.Subprotocol() == EventsSubprotocol {
		addr := h.addr
		if addr == "" {
			addr = r.RemoteAddr
		}
...
func TestJanusEventsHandlerNoMcu(t *testing.T) {
	...
	assert.Equal(EventsSubprotocol, response.Header.Get("Sec-WebSocket-Protocol"))
...
func TestJanusEventsHandlerNotGrouped(t *testing.T) {
	...
	metricstest.AssertCollectorChangeBy(t, statsJanusMediaNACKTotal.WithLabelValues("audio", "incoming"), 20)
	metricstest.AssertCollectorChangeBy(t, statsJanusMediaNACKTotal.WithLabelValues("audio", "outgoing"), 30)
	metricstest.AssertCollectorChangeBy(t, statsJanusMediaRetransmissionsTotal.WithLabelValues("audio"), 40)
	metricstest.AssertCollectorChangeBy(t, statsJanusMediaLostTotal.WithLabelValues("audio", "local"), 50)
	metricstest.AssertCollectorChangeBy(t, statsJanusMediaLostTotal.WithLabelValues("audio", "remote"), 60)
