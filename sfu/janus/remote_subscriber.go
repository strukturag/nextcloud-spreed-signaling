...
type janusRemoteSubscriber struct {
	...
	remote atomic.Pointer[janusRemotePublisher]
}

func (p *janusRemoteSubscriber) handleEvent(event *janus.EventMsg) {
	if videoroom := getPluginStringValue(event.Plugindata, pluginVideoRoom, "videoroom"); videoroom != "" {
		ctx := context.TODO()
		switch videoroom {
		case "destroyed":
			p.logger.Printf("Remote subscriber %d: associated room has been destroyed, closing", p.handleId.Load())
			go p.Close(ctx)
		case "event":
		...
		default:
			p.logger.Printf("Unsupported videoroom event %s for remote subscriber %d: %+v", videoroom, p.handleId.Load(), event)
		}
	} else {
		p.logger.Printf("Unsupported event for remote subscriber %d: %+v", p.handleId.Load(), event)
	}
}

func (p *janusRemoteSubscriber) handleHangup(event *janus.HangupMsg) {
	p.logger.Printf("Remote subscriber %d received hangup (%s), closing", p.handleId.Load(), event.Reason)
	go p.Close(context.Background())
}

func (p *janusRemoteSubscriber) handleDetached(event *janus.DetachedMsg) {
	p.logger.Printf("Remote subscriber %d received detached, closing", p.handleId.Load())
	go p.Close(context.Background())
}

func (p *janusRemoteSubscriber) handleConnected(event *janus.WebRTCUpMsg) {
	p.logger.Printf("Remote subscriber %d received connected", p.handleId.Load())
	p.mcu.SubscriberConnected(p.Id(), p.publisher, p.streamType)
}

func (p *janusRemoteSubscriber) handleSlowLink(event *janus.SlowLinkMsg) {
	if event.Uplink {
		p.logger.Printf("Remote subscriber %s (%d) is reporting %d lost packets on the uplink (Janus -> client)", p.listener.PublicId(), p.handleId.Load(), event.Lost)
	} else {
		p.logger.Printf("Remote subscriber %s (%d) is reporting %d lost packets on the downlink (client -> Janus)", p.listener.PublicId(), p.handleId.Load(), event.Lost)
	}
}

func (p *janusRemoteSubscriber) handleMedia(event *janus.MediaMsg) {
...
func (p *janusRemoteSubscriber) NotifyReconnected() {
	...
	if prev := p.handle.Swap(handle); prev != nil {
		if _, err := prev.Detach(context.Background()); err != nil {
			p.logger.Printf("Error detaching old remote subscriber handle %d: %s", prev.Id, err)
		}
	}
...
func (p *janusRemoteSubscriber) Close(ctx context.Context) {
	p.janusSubscriber.Close(ctx)
